...
try: validate_ai_plan({'schema_version':'1.0','summary':'Restart service safely to recover production availability'})
...
bad_plan = {
  "schema_version": "1.0",
  "summary": "Do something risky to production systems immediately",
...
```powershell
python -c "from a_oic.adapters.ai_plan_validator import validate_ai_plan; from a_oic.adapters.ai_adapter import AIPlanRejected;
try: validate_ai_plan({'schema_version':'1.0','summary':'Do something risky to production systems immediately','actions':[{'type':'self_destruct','target':'prod-db'}]})
except AIPlanRejected as e: print('REJECTED:', e)"
```

### Expected Output
```
REJECTED: AI plan schema violation: 'self_destruct' is not one of ['scale', 'rollback', 'restart', 'verify']
```
...
plan={'schema_version':'1.0','summary':'Restart service safely to recover production availability','actions':[{'type':'restart','target':'payments-api'}]};
...
- **Demonstrates AI suggestion ≠ execution (policy + contract gated)**
...
### Command
```powershell
python -c "from a_oic.adapters.ai_plan_validator import validate_ai_plan; from a_oic.orchestrate.executor import LocalOrchestrateStub, execute_plan;
plan=validate_ai_plan({'schema_version':'1.0','summary':'Restart service safely to recover production availability','actions':[{'type':'restart','target':'payments-api'}]});
out=execute_plan(incident_id='INC-DEMO-001',policy_decision='ALLOW',policy_reason='Low risk incident',validated_plan=plan,orchestrate=LocalOrchestrateStub());
...
### Command
```powershell
python -c "from a_oic.adapters.ai_plan_validator import validate_ai_plan; from a_oic.orchestrate.executor import execute_plan;
plan=validate_ai_plan({'schema_version':'1.0','summary':'Restart service safely to recover production availability','actions':[{'type':'restart','target':'payments-api'}]});
...
---
# MANUAL TEST 8 - Contract Version Missing (FAIL-CLOSED)

### What this validates

AI `cannot bypass contract versioning`.
...
```powershell
python -c "from a_oic.adapters.ai_plan_validator import validate_ai_plan; from a_oic.adapters.ai_adapter import AIPlanRejected;
plan={'summary':'Restart service safely to recover production availability','actions':[{'type':'restart','target':'payments-api'}]};
try: validate_ai_plan(plan)
except AIPlanRejected as e: print('REJECTED:', e)"
```

### Expected Output
```
REJECTED: Missing schema_version in AI plan
```
...
- **Enables safe future evolution**


---
# MANUAL TEST 9 - Unsupported Schema Version (REJECTED)

### What this validates

Future AI versions `cannot execute accidentally`.
...
{
  "schema_version": "2.0",
  "summary": "Restart service safely with new logic",
  "actions": [{"type": "restart", "target": "payments-api"}]
}
```
...
```powershell
python -c "from a_oic.adapters.ai_plan_validator import validate_ai_plan; from a_oic.adapters.ai_adapter import AIPlanRejected;
plan={'schema_version':'2.0','summary':'Restart service safely with new logic','actions':[{'type':'restart','target':'payments-api'}]};
try: validate_ai_plan(plan)
except AIPlanRejected as e: print('REJECTED:', e)"
```
...
### Expected Output
```
REJECTED: Unsupported AI plan schema_version: 2.0
```

### Why
- **Guarantees `forward incompatibility safety`**
- **Prevents AI rollout without kernel approval**
- **Matches enterprise API versioning norms**


---
# MANUAL TEST 10 - Supported Version (PASS)

### What this validates

Correctly versioned `AI output executes normally`.
...
### Expected Output
```
{'schema_version': '1.0', 'summary': 'Restart service safely to recover production availability', 'actions': [...]}
```

### Why

- **Confirms `version gate is not blocking valid plans`**
- **Proves `no regression introduced`**
- **Completes contract lifecycle proof**
...
| `test_ai_plan_acceptance.py` | Manual Tests 5 & 10 |
| `test_end_to_end_execution.py` | Manual Tests 6 & 10 |
| `test_orchestrate_adapter_switch.py` | Manual Test 7 |
| `test_contract_versioning.py` | Manual Tests 8 & 9 |
